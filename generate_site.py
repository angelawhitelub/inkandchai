...
<!-- Google Merchant Center / Search Console site verification (new account 5782474419) -->
  ...
  const makeCard = r => `
    <a class="rel-card" href="/product/${r.slug}/" style="text-decoration:none;display:block">
      <div class="rel-cover">
        ...
        ${r.n ? '<span style="position:absolute;top:6px;left:6px;background:var(--gold);color:var(--bg);font-size:0.42rem;letter-spacing:0.2em;padding:2px 6px;font-family:Montserrat,sans-serif;font-weight:700;text-transform:uppercase;">NEW</span>' : ''}
      </div>
...
html{{overflow-x:hidden}} *{{box-sizing:border-box;min-width:0}} body{{margin:0;overflow-x:hidden;background:var(--bg);color:var(--cream);font-family:Montserrat,sans-serif;font-weight:400}} a{{color:inherit}}
...
.cart-total-amount{{font-family:'Cormorant Garamond',serif;font-size:1.5rem;color:var(--gold);font-weight:600}}
.btn-checkout{{width:100%;font-family:'Montserrat',sans-serif;font-size:.65rem;letter-spacing:.25em;text-transform:uppercase;padding:1rem;background:var(--gold);color:var(--bg);border:none;cursor:pointer;font-weight:500;transition:all .3s}}
  ...
  _pinTimer = setTimeout(async () => {
    try {
      ...
      if (res.ok && data.state) {
        ...
        if (data.city) {
          msg.textContent = '✓ ' + data.city + ', ' + data.state;
