...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Bottom-LEFT on a phone put this circle directly on top of the left edge of every section CTA ("Shop the set", "Shop Hindi editions"): measured 46px of a 223px button covered, and at z-index 250 it took the tap. Bottom-right is clear of them. */
  @media(max-width:780px){.wa-float{bottom:88px;left:auto;right:14px;width:46px;height:46px;font-size:1.3rem}}
  /* A circle floating over a two-column grid covers a tap target wherever it sits: bottom-left it took the section CTAs, bottom-right it takes the right column's Add to Cart. So it yields while the page is actually being scrolled and comes back when it stops. */
...
(function(){
  ...
  if (!wa || !window.matchMedia || !matchMedia('(max-width:780px)').matches) return;
  var t;
  addEventListener('scroll', function(){
    wa.classList.add('wa-scrolling');
    clearTimeout(t);
    t = setTimeout(function(){ wa.classList.remove('wa-scrolling'); }, 550);
  }, { passive: true });
})();
</script>
...
/* Bottom-LEFT on a phone put this circle directly on top of the left edge of every section CTA ("Shop the set", "Shop Hindi editions"): measured 46px of a 223px button covered, and at z-index 250 it took the tap. Bottom-right is clear of them. */
@media(max-width:780px){.wa-float{bottom:84px;left:auto;right:14px;width:48px;height:48px}}
  /* A circle floating over a two-column grid covers a tap target wherever it sits: bottom-left it took the section CTAs, bottom-right it takes the right column's Add to Cart. So it yields while the page is actually being scrolled and comes back when it stops. */
  .wa-float{transition:transform .2s,box-shadow .2s,opacity .22s}
  .wa-float.wa-scrolling{opacity:0;pointer-events:none}
...
.wa-float:hover{transform:scale(1.08)}
/* Bottom-LEFT on a phone put this circle directly on top of the left edge of every section CTA ("Shop the set", "Shop Hindi editions"): measured 46px of a 223px button covered, and at z-index 250 it took the tap. Bottom-right is clear of them. */
@media(max-width:780px){.wa-float{bottom:88px;left:auto;right:14px;width:46px;height:46px}}
  /* A circle floating over a two-column grid covers a tap target wherever it sits: bottom-left it took the section CTAs, bottom-right it takes the right column's Add to Cart. So it yields while the page is actually being scrolled and comes back when it stops. */
  .wa-float{transition:transform .2s,box-shadow .2s,opacity .22s}
  .wa-float.wa-scrolling{opacity:0;pointer-events:none}
...
# ── PWA: service worker + offline page + registration ────────────────────────
#
# The Play Store build is a Trusted Web Activity: Android chrome around these
# same pages, so anything that happens here is what ships in the app. Two
# consequences drive every decision below.
#
# First, an installed app that shows Chrome's dinosaur when the train goes into
# a tunnel does not read as an app. Hence a real offline page and a page cache.
#
# Second, a wedged service worker on a shop is a revenue outage, and it is not
# something the shopkeeper can debug. So the worker refuses to involve itself in
# anything that takes money: no POSTs, no /.netlify/functions, no /checkout, no
# /admin. Those always go straight to the network, exactly as if no worker were
# installed. The worst a bug in here can do is serve a stale book cover.
#
# KILL SWITCH: replace the body of public/sw.js with
#   self.addEventListener('install',()=>self.skipWaiting());
#   self.addEventListener('activate',e=>e.waitUntil((async()=>{
#     for (const k of await caches.keys()) await caches.delete(k);
#     await self.registration.unregister();
#   })()));
# and deploy. Browsers re-fetch sw.js on navigation (it is served no-cache, see
# public/_headers), so every installed client uninstalls itself within a visit.

_SW_NEVER = "/admin/, /checkout/, /refund-upi/, /.netlify/"
_SW_JS = r"""/* Ink & Chai service worker — generated by generate_site.py, do not edit. */
...
# One stamp for the whole release. It is the hash of the worker's own source
# plus the two shell filenames, so a change to any of them renames every cache
# and the previous release's entries are deleted on activate.
_sw_precache = [
    "/offline/",
    "/manifest.json",
    "/images/icon-192.png",
    f"/css/{_shell_css_file}",
    f"/js/{_shell_js_file}",
]
_sw_body = _SW_JS.replace("__PRECACHE__", json.dumps(_sw_precache))
_sw_version = hashlib.md5(_sw_body.encode()).hexdigest()[:12]
_sw_body = _sw_body.replace("__VERSION__", _sw_version)
(_shell_public / "sw.js").write_text(_sw_body, encoding="utf-8")

# Registration. Deliberately not in the app-shell bundle: that script is skipped
# on product and checkout pages, and a product page is the most common entry
# point there is — the one page where a shopper arriving from an ad decides
# whether this is an app worth keeping.
_SW_REGISTER = (
    '<script>'
...
<meta charset="utf-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>No connection &mdash; Ink &amp; Chai</title>
<meta name="robots" content="noindex"/>
<link rel="manifest" href="/manifest.json"/>
<style>
  :root{--bg:#0d0b08;--fg:#faf7f2;--dim:#a99f92;--gold:#c9a84c;--line:#2a241c}
  @media (prefers-color-scheme:light){:root{--bg:#faf7f2;--fg:#1a1510;--dim:#6b6155;--line:#e2dad0}}
  *{box-sizing:border-box}
  body{margin:0;min-height:100vh;display:grid;place-items:center;padding:2rem;
       background:var(--bg);color:var(--fg);
       font-family:Manrope,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;text-align:center}
  .wrap{max-width:26rem}
  img{width:64px;height:64px;border-radius:14px;margin-bottom:1.5rem}
  h1{font-family:Georgia,'Times New Roman',serif;font-weight:400;font-size:1.6rem;margin:0 0 .75rem}
  p{color:var(--dim);line-height:1.6;margin:0 0 1.75rem;font-size:.95rem}
  button{font:inherit;font-size:.85rem;letter-spacing:.08em;text-transform:uppercase;
         padding:.85rem 2rem;border-radius:999px;border:1px solid var(--gold);
         background:transparent;color:var(--gold);cursor:pointer}
  button:active{background:rgba(201,168,76,.12)}
  .note{margin-top:1.75rem;padding-top:1.5rem;border-top:1px solid var(--line);
...
<body>
  <div class="wrap">
    ...
    <p>We could not reach Ink &amp; Chai. Your cart is saved on this device &mdash;
       nothing has been lost. Pages you have already opened will still work.</p>
    <button onclick="location.reload()">Try again</button>
    <div class="note">Orders already placed are unaffected. You can track them
...
"""
_offline_dir = _shell_public / "offline"
_offline_dir.mkdir(parents=True, exist_ok=True)
(_offline_dir / "index.html").write_text(_OFFLINE_HTML, encoding="utf-8")

# Every page except the admin panel. The panel is a different application that
# happens to share a domain, it is never installed, and it must never be served
# from a cache the shopper's worker controls.
_SW_OPEN, _SW_CLOSE = "<!--IAC-SW-->", "<!--/IAC-SW-->"
_sw_strip_re = re.compile(re.escape(_SW_OPEN) + r".*?" + re.escape(_SW_CLOSE) + r"\n?", re.S)
_sw_pages = 0
for _page in sorted(_shell_public.rglob("*.html")):
    if _page.relative_to(_shell_public).parts[0] == "admin":
        continue
    _h = _page.read_text(encoding="utf-8")
    if "</body>" not in _h:
        continue
    _new = _sw_strip_re.sub("", _h)
    _new = _new.replace("</body>", _SW_OPEN + _SW_REGISTER + _SW_CLOSE + "\n</body>", 1)
    if _new != _h:
        _page.write_text(_new, encoding="utf-8")
        _sw_pages += 1

print(f"Generated: /sw.js ({_sw_version}) + /offline/ — registered on {_sw_pages} pages")
