/**
 * Scheduled function: securely enqueue the long-running NimbusPost AWB worker.
 */
...
exports.handler = async () => {
  ...
  if (!secret) {
    console.error('[awb-sync-scheduler] ADMIN_SECRET is not configured');
    return { statusCode: 500, headers: HEADERS, body: JSON.stringify({ error: 'Scheduler auth not configured' }) };
  }

  try {
    const response = await fetch(`${site}/.netlify/functions/nimbuspost-awb-sync-background`, {
      method: 'POST',
    ...
    console.log(`[awb-sync-scheduler] worker enqueued (${response.status})`);
    return { statusCode: 200, headers: HEADERS, body: JSON.stringify({ enqueued: true }) };
  } catch (error) {
    console.error('[awb-sync-scheduler] failed:', error.message);
